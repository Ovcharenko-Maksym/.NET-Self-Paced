SELECT person.surname, person.name, COALESCE(SUM(order_details.price_with_discount * order_details.product_amount), 0) AS sum
FROM person
...
WHERE customer.person_id IS NULL
GROUP BY person.surname, person.name
ORDER BY sum ASC, person.surname ASC;